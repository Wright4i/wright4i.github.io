...
response = requests.put('http://127.0.0.1:5000/employees', json=new_employee)
if response.status_code == 200:
...
for department in departments:
    if department['manager'] == '101':
        ...
        patch_response = requests.patch(f"http://127.0.0.1:5000/departments/{department['id']}", json=department)
